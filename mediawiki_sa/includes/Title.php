...
#----------------------------------------------------------------------------
	...
	function getSubjectNsText() {
		global $wgContLang;
		return $wgContLang->getNsText( MyNamespace::getSubject( $this->mNamespace ) );
	}
	...
	function getTalkNsText() {
		global $wgContLang;
		return( $wgContLang->getNsText( MyNamespace::getTalk( $this->mNamespace ) ) );
	}
	 ...
	 */
	function canTalk() {
		return( MyNamespace::canTalk( $this->mNamespace ) );
	}
	 ...
	 */
	function isMovable() {
		return MyNamespace::isMovable( $this->getNamespace() )
			&& $this->getInterwiki() == '';
	}
	 ...
	 */
	function isTalkPage() {
		return MyNamespace::isTalk( $this->getNamespace() );
	}
	...
	/* private */ function secureAndSplit() {
		...
		do {
			if ( preg_match( "/^(.+?)_*:_*(.*)$/S", $t, $m ) ) {
				...
				if ( $ns = MyNamespace::getCanonicalIndex( $lowerNs ) ) {
					# Canonical namespace
					$t = $m[2];
					$this->mNamespace = $ns;
	...
	function getTalkPage() {
		return Title::makeTitle( MyNamespace::getTalk( $this->getNamespace() ), $this->getDBkey() );
	}
	...
	function getSubjectPage() {
		return Title::makeTitle( MyNamespace::getSubject( $this->getNamespace() ), $this->getDBkey() );
	}
